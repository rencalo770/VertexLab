...
{
    //分类
    ALL_CATEGORIES[AppType_Geometry] = "Geometry"; // 几何
    ALL_CATEGORIES[AppType_Topology] = "Topology"; // 拓扑
    ALL_CATEGORIES[AppType_Triangulation] = "Triangulation"; // 三角化 将图形分割成三角形
    ALL_CATEGORIES[AppType_DataExchange] = "DataExchange"; //数据导入、导出
    ALL_CATEGORIES[AppType_Ocaf] = "OCAF";
    ALL_CATEGORIES[AppType_Viewer2d] = "2D Viewer"; // 2d试图
    ALL_CATEGORIES[AppType_Viewer3d] = "3D viewer"; // 3d试图
...
//#include <Standard_WarningsRestore.hxx>
    ...
    //实例化示例
    myGeometrySamples = new GeometrySamples(aSampleSourcePach, myDocument3d->getContext());
    myTopologySamples = new TopologySamples(aSampleSourcePach, myDocument3d->getContext());
    myTriangulationSamples = new TriangulationSamples(aSampleSourcePach, myDocument3d->getContext());
    myDataExchangeSamples = new DataExchangeSamples(aSampleSourcePach, myGeomWidget->Get3dView(), myDocument3d->getContext());
    myOcafSamples = new OcafSamples(aSampleSourcePach, myDocument3d->getViewer(),  myDocument3d->getContext());
    myViewer3dSamples = new Viewer3dSamples(aSampleSourcePach, myGeomWidget->Get3dView(), myDocument3d->getContext());
    myViewer2dSamples = new Viewer2dSamples(aSampleSourcePach,  myGeomWidget->Get2dView(), myDocument2d->getViewer(),  myDocument2d->getContext());
...
/*
    ...
    */
    //暂时手动修改成可用路径"E:\\OpenCASCADE-7.7.0-vc14-64\\opencascade-7.7.0\\samples\\OCCTOverview\\code";
    TCollection_AsciiString aSampleSourceDir = "F:\\vsproject\\VertexLab\\source";
    return aSampleSourceDir;
}
...
void ApplicationCommonWindow::onProcessExchange(const QString& theSampleName)
{
    ...
    //手动重新设置为0，否则会导出失败
    aMode = 0;
    myDataExchangeSamples->SetStepType(static_cast<STEPControl_StepModelType>(0)); 
...
void ApplicationCommonWindow::MenuFormXml(const QString& thePath, QSignalMapper* theMapper, QList<QMenu*>& theMunusList)
{
